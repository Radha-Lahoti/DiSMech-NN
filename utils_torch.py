...
def signed_angle(u: torch.Tensor, v: torch.Tensor, n: torch.Tensor, eps: float = 1e-10) -> torch.Tensor:
    """
    u, v, n: (..., 3)
    returns: (...) signed angle from u to v about normal n
    """
    w = torch.cross(u, v, dim=-1)                  # (..., 3)
    norm_w = torch.linalg.norm(w, dim=-1)          # (...)

    dot_uv = torch.sum(u * v, dim=-1)              # (...)
    # NOTE: in your numpy code you used dot_uv as denominator for atan2(norm_w, dot_uv)
    # and clamped when near 0. We'll do the same.
    safe_dot = torch.where(dot_uv.abs() < eps, torch.ones_like(dot_uv), dot_uv)

    angle = torch.atan2(norm_w, safe_dot)          # (...)
    sign = torch.sign(torch.sum(n * w, dim=-1))    # (...)
...
def parallel_transport(u: torch.Tensor, t_start: torch.Tensor, t_end: torch.Tensor, eps: float = 1e-10) -> torch.Tensor:
    ...
    b_norm = torch.linalg.norm(b, dim=-1, keepdim=True)        # (..., 1)
    mask = (b_norm.squeeze(-1) < eps)                          # (...)

    safe_b_norm = torch.where(b_norm < eps, torch.ones_like(b_norm), b_norm)
    b_hat = b / safe_b_norm                                    # (..., 3)

    # orthogonalize against t_start
    dot_bt = torch.sum(b_hat * t_start, dim=-1, keepdim=True)  # (..., 1)
    b_ortho = b_hat - dot_bt * t_start

    b_ortho_norm = torch.linalg.norm(b_ortho, dim=-1, keepdim=True)
    safe_b_ortho_norm = torch.where(b_ortho_norm < eps, torch.ones_like(b_ortho_norm), b_ortho_norm)
    b_ortho_hat = b_ortho / safe_b_ortho_norm                  # (..., 3)
    ...
    # preserve original u where transport is unnecessary
    return torch.where(mask.unsqueeze(-1), u, comp)


def compute_tangent(q: torch.Tensor, edges) -> torch.Tensor:
    """
    q: (..., 3*num_nodes) or (..., num_nodes, 3) (we'll handle flat)
    edges: (E, 2) indices
    ...
    vecs = pos1 - pos0
    norms = torch.linalg.norm(vecs, dim=-1, keepdim=True)  # (..., E, 1)

    safe_norms = torch.where(norms < 1e-10, torch.ones_like(norms), norms)
    tangent = vecs / safe_norms
...
def compute_time_parallel(a1_old: torch.Tensor, q0: torch.Tensor, q: torch.Tensor, edges) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    a1_old: (..., num_nodes, 3) or (num_nodes, 3)
    q0, q:  (..., 3*num_nodes) or (..., num_nodes, 3)
    returns: (a1, a2) with shape matching a1_old
    """
    tangent0 = compute_tangent(q0, edges)  # (..., E, 3)  (if you knew you wanted per-node, adjust)
    tangent  = compute_tangent(q,  edges)

    # NOTE: Your original numpy version seems to call compute_tangent(q) without edges and treat it as per-node.
    # For rods, tangents are usually per-edge. If your a1_old is per-node, you must define how to map edge tangents to nodes.
    # If instead a1_old is per-edge, you're good.
    ...
    a1 = a1 / torch.clamp(torch.linalg.norm(a1, dim=-1, keepdim=True), min=1e-12)
