...
def compute_reference_twist(edges: np.ndarray,
                            ...
                            ref_twist: np.ndarray = None) -> np.ndarray:
    e0 = edges[:, 0]
    e1 = edges[:, 1]
...
def compute_time_parallel(a1_old: np.ndarray, q0: np.ndarray, q: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        tangent0 = compute_tangent(q0)
        tangent = compute_tangent(q)
...
def compute_material_directors(theta: np.ndarray, a1: np.ndarray, a2: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    cos_theta = np.cos(theta)[:, None]
    sin_theta = np.sin(theta)[:, None]
...
def compute_tangent(q: np.ndarray, edges: np.ndarray) -> np.ndarray:
    q = np.asarray(q)
    edges = np.asarray(edges)

    # reshape positions to (num_nodes, 3)
    X = q.reshape(-1, 3)
    ...
    vecs = pos1 - pos0                      # (E, 3)
    norms = np.linalg.norm(vecs, axis=1, keepdims=True)  # (E, 1)
    ...
    tangent[np.abs(tangent) < _TANGENT_THRESHOLD] = 0.0
